...
const videosList = [
  ...
  },
  {
    ...
    description:
      "My easy laksa curry recipe is perfect for when that noodle soup craving hits hard. Ready in 15 minutes!",
    views: "651K views",
  ...
  },
  {
    ...
    description:
      "No, this isn't instant noodles so don’t ask. I'm talking about the gourmet noodles that costs $15 or more for a single bowl! That's just unacceptable so I'll show you how to make homemade gourmet miso ramen for just 3 dollars that'll make all categories of ramen from fancy to instant look bad. ",
    views: "575K views",
  ...
  },
  {
    ...
    description:
      "6 New Ways To Enjoy Potatoes Recipes! 1. Braised Potatoes 2. Stir-Fried Potatoes 3. Korean Potato Pancake (Gamja Jeon) 4. Accordion Potato 5. Potato Cheese Ball 6. Potato Croquette",
    views: "1.6M views",
  ...
  {
    title:
      "👶 Ginger Egg Fried Rice (姜炒饭) | The Secret Fried Rice Recipe, Without Soy Sauce!",
    author: "Made with Lau",
    description:
      "Watch Daddy Lau teach us how to make this healthy, easy Ginger Egg Fried Rice recipe! You can do without soy sauce for this recipe :) ",
    views: "87K views",
  ...
  },
  {
    ...
    author: "Adam Ragusea",
    description:
      "I suppose this is more of a techniques than a recipe. Take whatever potatoes you want. Peel them, or don't. Cut them into fry-like shapes, being sure to get roughly equal thickness, and cut them maybe twice as thick as you want them — they'll shrink a lot during cooking.",
    views: "9M views",
  ...
  },
  {
    ...
    description:
      "If you're feeling like you've got a hectic morning and you would otherwise be skipping breakfast then we've got smoothie recipe ideas for you.",
    views: "1.9M views",
...
const lengthOfList = videosList.length;

function FNShowAllVideos() {
  let toReturn = ``;
  for (let i = 0; i < lengthOfList; i++)
    toReturn += generateRowHTML(videosList[i]);
  return toReturn;
}

function FNLookOutForSearchQuery(searchQuery) {
  let lengthOfSearchQuery = searchQuery.length;
  let toReturn = ``;
  for (let i = 0; i < lengthOfList; i++) {
    let lengthOfDiscription = videosList[i].description.length;
    for (let j = 0; j <= lengthOfDiscription - lengthOfSearchQuery; j++) {
      if (
        videosList[i].description.substring(j, j + lengthOfSearchQuery) ===
        searchQuery
      ) {
        toReturn += generateRowHTML(videosList[i]);
        break;
...
function FNHandleShowAllVideos() {
  let results = document.querySelector("#results");
  results.innerHTML = FNShowAllVideos();
}

function FNHandleSeachButton() {
  let searchQuery = document.querySelector("#txt-search").value;
  ...
  let searchResult = FNLookOutForSearchQuery(searchQuery);
  let results = document.querySelector("#results");
  if (searchResult.length == 0) {
    results.innerHTML = `No videos found with the keyword ` + searchQuery;
  } else {
    results.innerHTML = searchResult;
  }
}

let QSShowAllVideos = document.querySelector("#btn-show-all");
let QSSearchButton = document.querySelector("#btn-search");

QSShowAllVideos.addEventListener("click", FNHandleShowAllVideos);
QSSearchButton.addEventListener("click", FNHandleSeachButton);
